...
- **Half-written transcripts.** A turn normally ends with an assembled message under `diagnosticInfo`, and the streamed `sessionOutput.text` fragments that built it are skipped as duplicates. A session that ends abruptly never emits that assembled message for its last turns, so reading only the assembled ones silently drops the end of the conversation. Those turns are rebuilt from their fragments, and anything still unaccounted for is called out in a banner instead of quietly going missing.
